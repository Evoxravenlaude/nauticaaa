/**
 * ConfirmSeed — Step 2 of 3
 *
 * Verifies the user actually wrote down their seed phrase by asking them
 * to re-assemble it. Uses the real mnemonic from React Router location state.
 */
...
export default function ConfirmSeed() {
  ...
  // Guard: if someone navigates here directly without going through CreateWallet,
  // send them back so they can't skip the backup step.
  ...
  // Shuffle the real words for the picker
  const shuffled = useMemo(
    () => [...mnemonic].sort(() => Math.random() - 0.5),
    [mnemonic]
  );
  ...
  const handleWordClick = (word: string) => {
    ...
    setSelected(next);
    if (next.length === 12) {
      setError(!next.every((w, i) => w === mnemonic[i]));
    }
  };

  const handleReset = () => { setSelected([]); setError(false); };
      ...
      <div className="max-w-md mx-auto w-full mt-8">
        <div className="grid grid-cols-3 gap-2 min-h-[160px]">
          {Array.from({ length: 12 }).map((_, i) => (
            ...
            >
              <span className="font-mono text-[10px] text-text-tertiary block">{i + 1}</span>
              {selected[i] && (
