...
export default function Profile() {
  ...
  useEffect(() => {
    ...
    fetch(`https://api.ensideas.com/ens/resolve/${address}`, {
      ...
      .then((r) => r.json())
      .then((d: { name?: string }) => setEns(d.name ?? null))
      .catch(() => null);
  }, [address]);
      ...
      <div className="max-w-lg mx-auto px-4 py-8">
        ...
        <div className="bg-[#0C1220] border border-[#1A2540] p-6 mb-4 flex flex-col items-center text-center">
          <div className="w-20 h-20 rounded-full bg-gradient-to-br from-[#00F5D4]/20 to-purple-400/20 border border-[#1A2540] flex items-center justify-center mb-4">
            <svg width="40" height="40" viewBox="0 0 64 64" xmlns="http://www.w3.org/2000/svg"><defs><linearGradient id="pl" x1="0" y1="0" x2="1" y2="1"><stop offset="0%" stopColor="#06F5D6"/><stop offset="100%" stopColor="#0EA5E9"/></linearGradient></defs><circle cx="32" cy="32" r="30" fill="#0A1628"/><text x="32" y="42" textAnchor="middle" fontFamily="Arial Black,sans-serif" fontWeight="900" fontSize="32" fill="url(#pl)">N</text></svg>
          </div>
          {isConnected ? (
            <>
