...
export async function fetchTransactionHistory(address: string): Promise<AssetTransfer[]> {
  const makeBody = (fromAddr?: string, toAddr?: string) => ({
    ...
    params: [{
      ...
      withMetadata: true,
      maxCount:    '0x14',   // 20 each direction
      order:       'desc',
    }],
  });
  try {
    const [outRes, inRes] = await Promise.all([
      fetch(`${ALCHEMY_RPC}/v2/`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(makeBody(address, undefined)),
      }),
      fetch(`${ALCHEMY_RPC}/v2/`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(makeBody(undefined, address)),
      }),
    ]);
    const [outData, inData] = await Promise.all([
      outRes.json() as Promise<{ result?: { transfers?: AssetTransfer[] } }>,
      inRes.json()  as Promise<{ result?: { transfers?: AssetTransfer[] } }>,
    ]);
    const out = (outData.result?.transfers ?? []).map((t) => ({ ...t, direction: 'out' as const }));
    const inn = (inData.result?.transfers  ?? []).map((t) => ({ ...t, direction: 'in'  as const }));
    // Merge, deduplicate by hash+direction, sort descending by block
    const all = [...out, ...inn].reduce<AssetTransfer[]>((acc, t) => {
      if (!acc.find((x) => x.hash === t.hash && (x as AssetTransfer & { direction?: string }).direction === (t as AssetTransfer & { direction?: string }).direction)) acc.push(t);
      return acc;
    }, []);
    return all.sort((a, b) => parseInt(b.blockNum ?? '0x0', 16) - parseInt(a.blockNum ?? '0x0', 16));
  } catch {
    return [];
  }
}
...
// ─── Reservoir: NFT listings ─────────────────────────────────────────────────
const RESERVOIR = '/api/reservoir';
...
export async function fetchNFTListings(
  ...
  try {
    const r = await fetch(
      `${RESERVOIR}/orders/asks/v5?token=${contract}:${tokenId}&includeRawData=false&sortBy=price&limit=10`,
    );
    if (!r.ok) return [];
    const d = await r.json() as { orders?: Record<string, unknown>[] };
    return (d.orders ?? []).map((o) => {
      ...
      const currency = price?.currency as Record<string, unknown> | undefined;
      return {
        id:          (o.id as string) ?? '',
        price:       parseFloat((amount?.decimal as string) ?? '0'),
        currency:    (currency?.symbol as string) ?? 'ETH',
        marketplace: ((o.source as Record<string, unknown>)?.name as string) ?? 'Unknown',
        maker:       (o.maker as string) ?? '',
        expiration:  o.expiration ? parseInt(o.expiration as string) : null,
        tokenId,
        contract,
      };
    });
...
export interface ReservoirBuyStep {
  id:    string;
  action: string;
  items: Array<{ status: string; data?: Record<string, unknown> }>;
}

export async function fetchBuySteps(
  contract: string,
  tokenId:  string,
  taker:    string,
): Promise<ReservoirBuyStep[]> {
  try {
    const r = await fetch(`${RESERVOIR}/execute/buy/v7`, {
      method:  'POST',
      headers: { 'Content-Type': 'application/json' },
      body:    JSON.stringify({
        items:            [{ token: `${contract}:${tokenId}` }],
        taker,
        onlyPath:         false,
    ...
    if (!r.ok) return [];
    const d = await r.json() as { steps?: ReservoirBuyStep[] };
    return d.steps ?? [];
  } catch {
    return [];
  }
